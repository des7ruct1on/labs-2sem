...
void abiturientWriteBin(abiturient* s, FILE* out) {
    fwrite(s->surname, sizeof(char), MAXSIZE - 1, out);
    fwrite(s->initials, sizeof(char), MAXSIZE - 1, out);
    fwrite(s->sex, sizeof(char), MAXSIZE - 1, out);
    fwrite(&(s->schoolNum), sizeof(Item), 1, out);
    fwrite(s->medal, sizeof(char), MAXSIZE - 1, out);
    fwrite(s->composition, sizeof(char), MAXSIZE - 1, out);

    int numExams = sizeList(s->examStats);
    fwrite(&numExams, sizeof(Item), 1, out);

    node* current = s->examStats;
    while (current != NULL) {
        fwrite(current->data.subject, sizeof(char), MAXSIZE - 1, out);
        fwrite(&(current->data.ball), sizeof(int), 1, out);
        current = current->next;
    }
}
int abiturientReadBin(abiturient* s, FILE* in) {
    if (fread(s->surname, sizeof(char), MAXSIZE - 1, in) != MAXSIZE - 1)
        return 0;

    if (fread(s->initials, sizeof(char), MAXSIZE - 1, in) != MAXSIZE - 1)
        return 0;

    if (fread(s->sex, sizeof(char), MAXSIZE - 1, in) != MAXSIZE - 1)
        return 0;

    if (fread(&(s->schoolNum), sizeof(Item), 1, in) != 1)
        return 0;

    if (fread(s->medal, sizeof(char), MAXSIZE - 1, in) != MAXSIZE - 1)
        return 0;

    if (fread(s->composition, sizeof(char), MAXSIZE - 1, in) != MAXSIZE - 1)
        return 0;

    int numExams;
    if (fread(&numExams, sizeof(Item), 1, in) != 1)
        return 0;
    ...
    for (int i = 0; i < numExams; i++) {
        ...
        if (fread(subject, sizeof(char), MAXSIZE - 1, in) != MAXSIZE - 1)
            return 0;
